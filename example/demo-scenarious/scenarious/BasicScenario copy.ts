...
export interface BlankScenarioConfig {
...
export const configDefault: BlankScenarioConfig = {
  lineGroup_move_enabled: true,
  lineTextOptions_hover_enabled: false,
}
export const configStopMooving: BlankScenarioConfig = {
  lineGroup_move_enabled: false,
  lineTextOptions_hover_enabled: false,
}
export const configLineByLine: BlankScenarioConfig = {
  lineGroup_move_enabled: false,
  lineTextOptions_hover_enabled: false,
  lineByLine: true
}
export const configLineScreen: BlankScenarioConfig = {
  lineGroup_move_enabled: false,
  lineTextOptions_hover_enabled: false,
  lineScreen: true
}

export const configList: {[key: string]: BlankScenarioConfig} = {
  'default': configDefault,
  'stopMooving': configStopMooving,
  'lineByLine': configLineByLine,
  'lineScreen': configLineScreen,
}

/**
 * BlankScenario class
 */
export class BlankScenario extends MathreeqScenario<BlankScenarioOptions> {

  actionInterval: Interval | null = null;

  config: BlankScenarioConfig = configDefault;

  counter: number = 0;

  xToAppearStack: {[key: number]: Timeout | null} = {};

  constructor(name: string, mathreeq: Mathreeq, options: BlankScenarioOptions) {
  ...
  getPositionToAppearForLineScreen(): THREE.Vector3 | null {
    const pointToAppear = this.options.areaToolAppear.getRandomPointWithStep(this.options.areaPointStep);
    if (this.xToAppearStack[pointToAppear.x]) {
      if (this.xToAppearStack[pointToAppear.x] == null) {
  ...
  getPositionToAppear(): THREE.Vector3 | null {
    if (this.config['lineByLine']) {
      // #TODO: pregenerate. or via areaTool callback. but let it be...
    ...
    if (this.config['lineScreen']) {
      return this.getPositionToAppearForLineScreen();
    }
  ...
  runRandomTag(): void {
    ...
    // do smtn for lineScreen
    this.xToAppearStack[x] = this.mathreeq.intervaq.setTimeout(() => {
      this.xToAppearStack[x] = null;
    }, cellItem_lifeTime)
